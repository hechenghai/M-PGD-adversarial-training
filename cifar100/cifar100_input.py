...
class CIFAR10Data(object):
    ...
    def __init__(self, path):
        ...
        train_images, train_labels = self._load_datafile(os.path.join(path, train_filename),50000)
        eval_images, eval_labels = self._load_datafile(os.path.join(path, eval_filename),10000)
        ...
        #print(x.shape)
        #print(y.shape)
        #self.train_data = DataSubset(x,y)
        #self.eval_data = DataSubset(x_test,y_test)


    @staticmethod
    def _load_datafile(filename,number):
      with open(filename, 'rb') as fo:
          ...
          image_data = image_data.reshape((number, 3, 32, 32)).transpose(0, 2, 3, 1)
