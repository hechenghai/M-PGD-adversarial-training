...
class LinfPGDAttack:
  def __init__(self, model, epsilon, num_steps, step_size, random_start, loss_func, momentum):
    ...
    if loss_func == 'xent':
      ...
      self.loss = model.xent
    self.mean_loss = model.mean_xent
  ...
  def perturb(self, x_nat, y, sess):
    ...
    eps = self.epsilon
    alpha = eps / self.num_steps
    noise = 0
    for i in range(self.num_steps):
      grad = sess.run(self.grad, feed_dict={self.model.x_input: x,
                                            self.model.y_input: y})
      noise = self.momentum * noise + grad 
      x = np.add(x, alpha * np.sign(noise), out = x, casting='unsafe')
